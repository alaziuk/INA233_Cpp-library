...
class INA233 {
    public:
     INA233(int bus, uint8_t address = INA233_ADDRESS_40);
     ~INA233();
     void calibrate(double R_shunt, double I_max);
     int16_t getBusVoltageIn_raw();
     int16_t getBusVoltageOut_raw();
     int16_t getShuntVoltage_raw();
     int16_t getCurrentIn_raw();
     int16_t getCurrentOut_raw();
...
/* TODO LIST JUST FOR ME*/
/*
- Znaleźć libkę do I2C w cpp (DONE)
- Type Safe Registers implementacja do rejestrow (moze inna libke?)
- Pozamieniać static_cast w razie bugów
- Funkcja i2c_smbus_read_word_data wypluwa signed int, potencjalny problem (static_cast<int16_t>()?? Józek pomoze)
